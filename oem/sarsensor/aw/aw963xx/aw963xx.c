...
#define AW963XX_DRIVER_VERSION "v0.1.1.1"
		...
		char *buf)
{
	...
	if (p_sar->last_mode == AW963XX_SLEEP_MODE)
		mode = AW963XX_SLEEP_AP_MODE;
	else if (p_sar->last_mode == AW963XX_ACTIVE_MODE)
		mode = AW963XX_ACTIVE_AP_MODE;
	else if (p_sar->last_mode == AW963XX_DEEPSLEEP_MODE)
		mode = AW963XX_DEEPSLEEP_AP_MODE;
...
{
	int8_t mode = 0;
	enum aw963xx_operation_mode set_mode = 0;
	struct aw963xx *aw963xx = container_of(class, struct aw963xx, capsense_class);
	struct aw_sar *p_sar = NULL;
...
/*
	for (i = 0; i < AW963XX_CHANNEL_NUM_MAX; i++) {
		...
		AWLOGD(p_sar->dev, "enable cap sensor: %s\n", buf);
	}
*/
	if (mode == AW963XX_SLEEP_AP_MODE)
		set_mode = AW963XX_SLEEP_MODE;
	else if (mode == AW963XX_ACTIVE_AP_MODE)
		set_mode = AW963XX_ACTIVE_MODE;
	else if (mode == AW963XX_DEEPSLEEP_AP_MODE)
		set_mode = AW963XX_DEEPSLEEP_MODE;
		...
		char *buf)
{
	...
	if(aw963xx->read_flag){
		...
		AWLOGD(p_sar->dev, "read_reg = 0x%x, val = 0x%x\n", aw963xx->read_reg, *p);
		return 4;
	}
		...
		const char *buf, size_t count)
{
	...
	if( count != 7){
		AWLOGD(p_sar->dev, "params error[ count == %lu !=2]\n", count);
		return -1;
	}
	for(i = 0 ; i < count ; i++)
...
{
	uint8_t i = 0;
	struct aw963xx *aw963xx = container_of(sensors_cdev, struct aw963xx, sensors_capsensor_chs[0]);
	...
	for (i = 0; i < AW963XX_CHANNEL_NUM_MAX; i++) {
		...
		AWLOGI(p_sar->dev, "enable cap sensor: %s", sensors_cdev->name);
	}

	if (mode == AW963XX_SLEEP_AP_MODE)
		set_mode = AW963XX_SLEEP_MODE;
	else if (mode == AW963XX_SLEEP_AP_MODE)
		set_mode = AW963XX_ACTIVE_MODE;
	else if (mode == AW963XX_DEEPSLEEP_AP_MODE)
		set_mode = AW963XX_DEEPSLEEP_MODE;

	aw_sar_mode_set(p_sar, enable);

	return 0;
}

static const char *g_aw963xx_ch_name[] = {
	"Moto CapSensor Ch0", "Moto CapSensor Ch1", "Moto CapSensor Ch2",
	"Moto CapSensor Ch3", "Moto CapSensor Ch4", "Moto CapSensor Ch5",
	"Moto CapSensor Ch6", "Moto CapSensor Ch7", "Moto CapSensor Ch8"
...
static int32_t aw_sar_custom_flie_node_create(void *data)
{
	...
	for (i = 0; i < AW963XX_CHANNEL_NUM_MAX; i++){
		...
		AWLOGI(p_sar->dev, "cap sensor_class channel_name:%s", g_AW963XX_ch_name[g_aw963xx_counter]);
...
int32_t aw963xx_init(struct aw_sar *p_sar)
{
	...
	if (0)
		aw9630x_abnormal_irq_timer_call_func(1);
