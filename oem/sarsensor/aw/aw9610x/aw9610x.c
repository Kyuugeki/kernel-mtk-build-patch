...
static void aw9610x_irq_handle(struct aw_sar *p_sar)
{
	...
	for (i = 0; i < AW9610X_CHANNEL_MAX; i++) {
		...
		case AW9610X_TRIGGER_TH0:
			AWLOGD(p_sar->dev, "AW9610X_TRIGGER_TH0");
...
int32_t aw9610x_check_chipid(void *data)
{
	...
	if (reg_val == AW9610X_CHIP_ID) {
		...
		FULL_PRODUCT_DEVICE_INFO(ID_SAR_SENSOR,"AW96105A");
		#endif
		return AW_OK;
	} else {
		...
		const char *buf, size_t count)
{
	...
	if (g_aw_sar == NULL) {
		AWLOGE(g_aw_sar->dev, "cali start fail\n");
		return 0;
	}
	if (sscanf(buf, "%d", &cali_flag) != 1)
		return count;
	if (cali_flag == 1){
	AWLOGE(g_aw_sar->dev, "cali start\n");
	aw9610x_set_aot(g_aw_sar);

	}
	AWLOGE(g_aw_sar->dev, "cali_store mode=%d\n",cali_flag);
...
static int32_t aw_sar_custom_flie_node_create(void *data)
{
	...
	aw9610x = (struct aw9610x *)p_sar->priv_data;

	AWLOGD(p_sar->dev, "aw_sar_custom_flie_node");

	aw9610x->capsense_class.name = "capsense";
	aw9610x->capsense_class.owner = THIS_MODULE;

	ret = class_register(&aw9610x->capsense_class);
	if (ret < 0) {
		AWLOGE(p_sar->dev, "Create fsys class failed (%d)\n", ret);
		return ret;
	}

	ret = class_create_file(&aw9610x->capsense_class, &class_attr_cali);
	if (ret < 0) {
...
static void aw_sar_custom_flie_node_free(void *data)
{
	...
	class_remove_file(&aw9610x->capsense_class, &class_attr_cali);

	class_unregister(&aw9610x->capsense_class);
}
#endif
...
static const struct aw_sar_chip_config g_aw9610x_chip_config = {
	...
	.p_other_operation_sec = aw_sar_custom_flie_node_create,
