...
#include "aw_sar.h"
#include "./aw9610x/aw9610x.h"
#include <linux/notifier.h>

#define AW_SAR_I2C_NAME		"awinic_sar"
#define AW_SAR_DRIVER_VERSION	"v0.1.5.1"
...
//TN Begin modified by bingtai.zou/860558 20220818 EKFOGO4G-1520 begin
struct aw_pad {
	uint8_t *name;
	__u16 vendor;
};
...
//TN Begin modified by bingtai.zou/860558 20220818 EKFOGO4G-1520 end
//TN Begin modified by bingtai.zou/860558 20230822 EKFOGO4G-1543 Begin
#define USB_POWER_SUPPLY_NAME			"ext_charger_type"
//TN Begin modified by bingtai.zou/860558 20230822 EKFOGO4G-1543 End
...
//TN Begin modified by bingtai.zou/860558 20220113 EKFOGO4G-1560 begin
bool CA_FLAG = FALSE;
//TN Begin modified by bingtai.zou/860558 20220113 EKFOGO4G-1560 end
...
static int32_t aw_sar_parse_bin(const struct firmware *cont, struct aw_sar *p_sar)
{
	...
	//AWLOGE(p_sar->dev, "Bin file size: %d  aw_bin=%p", (uint32_t)cont->size, aw_bin);
		...
		if (ret != AW_OK) {
			...
			if (p_sar->load_bin.bin_load_fail_opera_func != NULL) {
				...
				if (ret != AW_OK) {
					...
					AWLOGE(p_sar->dev, "bin_load_fail_opera_func OK!");
				}
			} else {
				goto err;
			}
...
int32_t aw_sar_load_bin_comm(struct aw_sar *p_sar)
{
	...
	AWLOGE(p_sar->dev, "[TN]name :%s", p_sar->load_bin.bin_name);

	ret = request_firmware(&fw, p_sar->load_bin.bin_name, p_sar->dev);
	AWLOGE(p_sar->dev, "ret = %d", ret);
	if (ret != AW_OK) {
		AWLOGE(p_sar->dev, "parse %s error!", p_sar->load_bin.bin_name);
		return -AW_ERR;
	} else {
		AWLOGE(p_sar->dev, "parse %s ok!", p_sar->load_bin.bin_name);
	}
	...
	if (ret != AW_OK) {
		...
		AWLOGE(p_sar->dev, "reg_bin %s load ok!",p_sar->load_bin.bin_name);
	}
	...
	if (gpio_is_valid(p_sar->dts_info.irq_gpio)) {
		...
		if (ret) {
			...
			if (ret != 0) {
				...
				ret = AW_OK;
				p_sar->ret_val = AW_ERR_INTERRUPT_APPLIY;
			}
		}
	} else {
...
static int32_t aw_sar_input_init_comm(struct aw_sar *p_sar)
{
	...
	for (i = 0; i < p_sar->p_sar_para->ch_num_max; i++) {
		...
		if ((p_sar->dts_info.channel_use_flag >> i) & 0x01) {
			...
			//TN Begin modified by bingtai.zou/860558 20220818 EKFOGO4G-1520 begin
			p_sar->channels_arr[i].input->name = pad_event[i].name;
			p_sar->channels_arr[i].input->id.vendor = pad_event[i].vendor;
...
static int32_t aw_sar_chip_other_operation_sec(struct aw_sar *p_sar)
{
	if (p_sar->p_sar_para->p_other_operation_sec != NULL) {
		return p_sar->p_sar_para->p_other_operation_sec(p_sar);
	}
	return AW_OK;
}
...
int32_t aw_sar_load_def_reg_bin(struct aw_sar *p_sar)
{
	...
	if (CA_FLAG == TRUE) {
		...
		"%s_ca_%d.bin", p_sar->p_sar_para->p_reg_bin->bin_name, p_sar->dts_info.sar_num);
	} else {
		snprintf(p_sar->load_bin.bin_name, sizeof(p_sar->load_bin.bin_name),
		"%s_noca_%d.bin", p_sar->p_sar_para->p_reg_bin->bin_name, p_sar->dts_info.sar_num);
	}
	//TN Begin modified by bingtai.zou/860558 20220113 EKFOGO4G-1560  end
		...
		unsigned long event, void *p)
{
	...
	AWLOGE(p_sar->dev, "ps notification: event = %lu name=%s", event, psy->desc->name);
	//TN Begin modified by bingtai.zou/860558 20230822 EKFOGO4G-1543 Begin
	if ((event == PSY_EVENT_PROP_CHANGED) && psy && psy->desc->get_property && psy->desc->name &&
		(!strncmp(psy->desc->name, USB_POWER_SUPPLY_NAME, sizeof(USB_POWER_SUPPLY_NAME)))) {
	//TN Begin modified by bingtai.zou/860558 20230822 EKFOGO4G-1543 End
		retval = aw_sar_ps_get_state(p_sar, psy, &present);
		AWLOGE(p_sar->dev, "retval = %d", retval);
		if (retval) {
...
int32_t aw_sar_chip_init(struct aw_sar *p_sar)
{
	int32_t ret = 0;
	//TN Begin modified by bingtai.zou/860558 20220113 EKFOGO4G-1560 begin
	unsigned int ca_value = 0;
	ca_value = oem_pcba_ca();
	if (ca_value > 0) {
		CA_FLAG = TRUE;
	} else {
		CA_FLAG = FALSE;
	...
	ret = aw_sar_chip_other_operation_sec(p_sar);
