...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    case GESTURE_SINGLECLICK:
        gesture = KEY_GESTURE_TAP1;
        break;
    case GESTURE_DOUBLECLICK:
        gesture = KEY_GESTURE_TAP2;
        break;
    case GESTURE_O:
...
void fts_gesture_recovery(struct fts_ts_data *ts_data)
{
    if (ts_data->gesture_support && ts_data->suspended) {
        ...
        if (ts_data->gesture_tpye == GESTURE_SINGLE)
            fts_write_reg(0xCF, 0x01);
        if (ts_data->gesture_tpye == GESTURE_DOUBLE)
            fts_write_reg(0xCF, 0x02);
        if (ts_data->gesture_tpye == GESTURE_SINGLE_DOUBLE)
            fts_write_reg(0xCF, 0x03);
#endif
    }
}
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    FTS_FUNC_ENTER();
    input_set_capability(input_dev, EV_KEY, KEY_GESTURE_TAP2);
    input_set_capability(input_dev, EV_KEY, KEY_GESTURE_TAP1);
    ...
    __set_bit(KEY_GESTURE_TAP2, input_dev->keybit);
    __set_bit(KEY_GESTURE_TAP1, input_dev->keybit);
