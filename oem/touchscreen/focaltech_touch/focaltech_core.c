...
#if IS_ENABLED(CONFIG_OEM_DEVINFO)
static int fts_get_tp_info(char *buf, void *arg0)
{
    u8 val = 0;
    fts_read_reg(FTS_REG_FW_VER, &val);

    return sprintf(buf,"%s-%s-%s-0X%02x\n",
        "DJIN", "P325A", "FT8057S", val);
}
#endif
...
static int fts_charger_notifier_callback(struct notifier_block *nb, unsigned long val, void *v)
{
    ...
    psy = power_supply_get_by_name("ext_charger_type");
    ...
    if (!(strcmp(psy->desc->name, "ext_charger_type"))) {
        if (psy && val == POWER_SUPPLY_PROP_STATUS) {
            ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_ONLINE, &prop);
            if (ret < 0) {
                FTS_INFO("Couldn't get POWER_SUPPLY_PROP_ONLINE rc=%d\n", ret);
                return ret;
            } else {
                if (ts_data->usb_plug_status == 2)
                    ts_data->usb_plug_status = prop.intval;
                if (ts_data->usb_plug_status != prop.intval) {
                    FTS_INFO("usb prop.intval =%d psy->desc->name =%s\n",
                                prop.intval, psy->desc->name);
                    ts_data->usb_plug_status = prop.intval;
                    if (!ts_data->suspended && (ts_data->charger_notify_wq != NULL))
                        queue_work(ts_data->charger_notify_wq, &ts_data->update_charger);
                }
            }
        }
    }
...
static int fts_write_charger_reg(u8 value)
{
    int ret = 0;
    u8 m_val = 0;
...
static void fts_update_charger(struct work_struct *work)
{
    ...
    if (ts_data->usb_plug_status) {
        ...
        ret = fts_write_charger_reg(ENABLE);
        if (ret >= 0) {
            ts_data->charger_mode = ENABLE;
            FTS_INFO("ts_data->charger_mode = %d", ts_data->charger_mode);
        }
    } else if (!(ts_data->usb_plug_status)) {
        FTS_INFO("exit charger mode");
        ret = fts_write_charger_reg(DISABLE);
        if (ret >= 0) {
...
int fts_plat_charger_init(void)
{
    int ret = 0;
    struct fts_ts_data *ts_data = fts_data;
    ...
    INIT_WORK(&ts_data->update_charger, fts_update_charger);
...
int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    ret = touch_info_node_init();
    if (ret) {
        FTS_ERROR("touch_info_node_init fail");
    }
#endif

#if CHARGER_NOTIFIER_CALLBACK
#if KERNEL_VERSION(4, 1, 0) <= LINUX_VERSION_CODE
    ret = fts_plat_charger_init();
    if (ret) {
        FTS_ERROR("ilitek_plat_charger_init fail");
    }
#endif
#endif

#if IS_ENABLED(CONFIG_OEM_DEVINFO)
    FULL_PRODUCT_DEVICE_CB(ID_TP, fts_get_tp_info, NULL);
#endif
