...
#define FTS_GESTURE_ON(flag)      (flag[0] == '3')
#define FTS_DOUBLE_TAP_ON(flag)   (flag[0] == '2')
#define FTS_SINGLE_TAP_ON(flag)   (flag[0] == '1')
#define FTS_GESTURE_OFF(flag)     (flag[0] == '0')
#define FTS_GESTURE_JUDGE(flag)   (flag[0] != '0')

int fts_lcd_gesture_control(void)
{
    struct fts_ts_data *ts_data = fts_data;

    if(ts_data->gesture_support) {
        FTS_INFO("FT8057S LCM gesture on !\n");
        return gesture_on;
    }
    else if (!(ts_data->gesture_support)) {
        FTS_INFO("FT8057S LCM gesture off !\n");
        return gesture_off;
    }
    else
        return -EINVAL;
}
EXPORT_SYMBOL_GPL(fts_lcd_gesture_control);

int fts_tp_selftest(void)
{
    int ret = 0;
    char fwname[128] = {0};
    struct fts_ts_data *ts_data = fts_data;
    ...
    FTS_INFO("fts_tp_selftest enter !\n");
...
static ssize_t tp_selftest_result_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
    ...
    mutex_lock(&fts_data->touch_mutex);
    ...
    *pos += len;
    mutex_unlock(&fts_data->touch_mutex);
    return len;
...
static ssize_t tp_gesture_mode_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
    ...
    if (ts_data->suspended) {
        FTS_INFO("In suspend, can't echo, return now");
        return -EINVAL;
    }

    if ((size) > sizeof(cmd)) {
        FTS_ERROR("ERROR! input length is larger than local buffer\n");
        return -1;
    }
    ...
    if (buff != NULL) {
        if (copy_from_user(cmd, buff, size)) {
            FTS_ERROR("Failed to copy data from user space\n");
            size = -1;
            goto out;
        }
    }

    ts_data->gesture_support = (FTS_GESTURE_JUDGE(cmd)) ? ENABLE:DISABLE;	

    if (FTS_GESTURE_ON(cmd)) {
        ts_data->gesture_tpye = GESTURE_SINGLE_DOUBLE;
    } else if (FTS_DOUBLE_TAP_ON(cmd)) {
        ts_data->gesture_tpye = GESTURE_DOUBLE;
    } else if (FTS_SINGLE_TAP_ON(cmd)) {
        ts_data->gesture_tpye = GESTURE_SINGLE;
    } else if (FTS_GESTURE_OFF(cmd)) {
        ts_data->gesture_tpye = GESTURE_DISABLE;
    } else {
        FTS_INFO("error cmd %s!\n", cmd);
        goto out;
    }
	
	FTS_INFO("ts_data->gesture_support = %d ,ts_data->gesture_tpye = %d\n",
			ts_data->gesture_support,ts_data->gesture_tpye);
out:
    mutex_unlock(&fts_data->touch_mutex);
    return size;
...
int touch_info_node_init(void)
{
    int i = 0;
    FTS_INFO("touch_info_node_init\n");
    proc_dir_fts_tp_info = proc_mkdir("touch_info", NULL);
    for (; i < ARRAY_SIZE(tp_info_proc); i++) {
        tp_info_proc[i].node = proc_create(tp_info_proc[i].name, 0644,
                    proc_dir_fts_tp_info, tp_info_proc[i].fops);
        if (tp_info_proc[i].node == NULL) {
            tp_info_proc[i].isCreated = false;
            FTS_ERROR("Failed to create %s under /proc\n", tp_info_proc[i].name);
            return -ENOMEM;
        } else {
            tp_info_proc[i].isCreated = true;
            FTS_INFO("Succeed to create %s under /proc\n", tp_info_proc[i].name);
        }
    }
    return 0;
}
