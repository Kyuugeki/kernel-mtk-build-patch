...
int ili_lcd_gesture_control(void)
{
	if(ilits->gesture) {
		ILI_INFO("ILI9883C LCM gesture on !\n");
		return gesture_on;
	}
	else if (!(ilits->gesture)){
		ILI_INFO("ILI9883C LCM gesture off !\n");
		return gesture_off;
	}
	else
		return -EINVAL;
}
EXPORT_SYMBOL_GPL(ili_lcd_gesture_control);
