...
static int sc8989x_get_charger_type(struct sc8989x_chip *sc)
{
	...
	case VBUS_STAT_UNKOWN:
		sc->psy_usb_type = POWER_SUPPLY_USB_TYPE_UNKNOWN;
		sc->chg_type = POWER_SUPPLY_TYPE_UNKNOWN;
		sc->psy_desc.type = POWER_SUPPLY_TYPE_USB_TYPE_C;
...
static irqreturn_t sc8989x_irq_handler(int irq, void *data)
{
	int ret;
	int reg_val;
	bool prev_vbus_gd, prev_power_gd;
	...
	if (ret) {
		dev_err(sc->dev, "%s: get power good state failed(%d)\n", __func__, ret);
		return IRQ_HANDLED;
	} else {
		dev_info(sc->dev, "%s: power_gd = 0x%x\n", __func__, reg_val);
		prev_power_gd = sc->power_good;
	...
	if (sc->force_detect_count != 0 && !prev_power_gd && sc->power_good) {
		dev_info(sc->dev, "%s: bc1.2 detect done, start get charger type\n", __func__);
		sc8989x_get_charger_type(sc);
		power_supply_changed(sc->psy);
	}
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		if (val->intval == 2) {
			dev_info(sc->dev, "%s: %d, ignore BC1.2 due to it is being started in irq handler\n", __func__, val->intval);
