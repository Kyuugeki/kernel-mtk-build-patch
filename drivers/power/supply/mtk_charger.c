...
#define QC_TARGE_VOLT  6000 //mV
#define QC_MAX_VOLT    (QC_TARGE_VOLT + 200) //mV
int qc_charging_mode = 0;
...
#define STOP_CHARGING_SOC		80
#define RESUME_CHARGING_SOC		20
/*TN End modified by zelin.pan/860620 20230823 CR/EKFOGO4G-1548*/

/*TN Begin modified by zelin.pan/860620 20230911 CR/EKFOGO4G-1036*/
#if IS_ENABLED(CONFIG_OEM_DEVINFO)
#include <dev_info.h>
#endif
/*TN End modified by zelin.pan/860620 20230911 CR/EKFOGO4G-1036*/
...
static unsigned int factory_charging_limit = 1;
#endif

/*TN Begin modified by zelin.pan/860620 20230823 CR/EKFOGO4G-1548*/
static unsigned int  battery_protection_mode = 0;
static unsigned int  demo_mode_limit = 0;
static unsigned int  turbo_power_mode = 0;
static unsigned int  turbo_test_mode = 0;
/*TN End modified by zelin.pan/860620 20230823 CR/EKFOGO4G-1548*/

/*TN Begin modified by hao.jia/809321 20230908 CR/EKFOGO4G-1677*/
#if IS_ENABLED(CONFIG_OEM_TURBO_CHARGER)
bool qc_logic_probe_done = 0;
EXPORT_SYMBOL(qc_logic_probe_done);
/*TN Begin modified by hao.jia/809321 20230920 CR/EKFOGO4G-2046*/
bool mtk_can_charging = true;
EXPORT_SYMBOL(mtk_can_charging);
/*TN End modified by hao.jia/809321 20230920 CR/EKFOGO4G-2046*/
...
/*TN Begin modified by zelin.pan/860620 20231201 CR/EKFOGO4G-8482*/
int qc3p_charger_ready = 0;
EXPORT_SYMBOL(qc3p_charger_ready);
/*TN End modified by zelin.pan/860620 20231201 CR/EKFOGO4G-8482*/
/*TN End modified by zhen.liu11/860655 20230922 CR/EKFOGO4G-1483*/

extern bool is_turbo_charger_ready;

#endif
/*TN End modified by hao.jia/809321 20230908 CR/EKFOGO4G-1677*/
...
/*TN Begin modified by zhen.liu11/860655 20230925 CR/EKFOGO4G-1886*/
bool is_disable_charger(struct mtk_charger *info);
/*TN End modified by zhen.liu11/860655 20230925 CR/EKFOGO4G-1886*/
...
/*TN Begin modified by zhen.liu11/860655 20230925 CR/EKFOGO4G-1886*/
	if((is_disable_charger(info) == false) || is_battery_exist(info)) {
...
/*TN Begin modified by zelin.pan/860620 20230911 CR/EKFOGO4G-1036*/
	if (of_property_read_u32(np, "charger_configuration", &val) >= 0) {
		if (!oem_pcba_chg_15w_exist())
			...
			val = SINGLE_CHARGER;
		chr_err("use charger_configuration:%d\n", val);
		info->config = val;
	} else {
/*TN End modified by zelin.pan/860620 20230911 CR/EKFOGO4G-1036*/
	...
	/*TN Begin modified by hao.jia/809321 20231009 CR/EKFOGO4G-1548*/
	if (of_property_read_s32(np, "temp_t0_thres", &temp) >= 0)
		info->data.temp_t0_thres = temp;
	else {
	...
	if (of_property_read_s32(np, "temp_t0_thres_plus_x_degree", &temp) >= 0)
		info->data.temp_t0_thres_plus_x_degree = temp;
	else {
	...
	if (of_property_read_s32(np, "temp_neg_10_thres", &temp) >= 0)
		info->data.temp_neg_10_thres = temp;
	else {
		chr_err("use default TEMP_NEG_10_THRES:%d\n",
			TEMP_NEG_10_THRES);
		info->data.temp_neg_10_thres = TEMP_NEG_10_THRES;
	}
	/*TN End modified by hao.jia/809321 20231009 CR/EKFOGO4G-1548*/
	...
	/*TN Begin modified by lingfei.tang 20231101 CR/EKFOGO4G-5917*/
	if (of_property_read_s32(np, "min_charge_temp", &temp) >= 0)
		info->thermal.min_charge_temp = temp;
	else {
	...
	if (of_property_read_s32(np, "min_charge_temp_plus_x_degree", &temp)
		>= 0) {
		info->thermal.min_charge_temp_plus_x_degree = temp;
	/*TN End modified by lingfei.tang 20231101 CR/EKFOGO4G-5917*/
	} else {
...
/*TN Begin modified by zelin.pan/860620 20230916 CR/EKFOGO4G-2062*/
#if IS_ENABLED(QC_CHARGING_MODE)
	if (of_property_read_u32(np, "qc_temp_above_t4_icurrent", &val) >= 0)
		info->data.qc_temp_above_t4_icurrent = val;
	else {
		chr_err("use default qc_temp_above_t4_icurrent:0\n");
		info->data.qc_temp_above_t4_icurrent = 0;
	}

	if (of_property_read_u32(np, "qc_temp_t3_to_t4_icurrent", &val) >= 0)
		info->data.qc_temp_t3_to_t4_icurrent = val;
	else {
		chr_err("use default qc_temp_t3_to_t4_icurrent:%d\n",
			QC_CHARGER_CURRENT);
		info->data.qc_temp_t3_to_t4_icurrent = QC_CHARGER_CURRENT;
	}

	if (of_property_read_u32(np, "qc_temp_t2_to_t3_icurrent", &val) >= 0)
		info->data.qc_temp_t2_to_t3_icurrent = val;
	else {
		chr_err("use default qc_temp_t2_to_t3_icurrent:%d\n",
			QC_CHARGER_CURRENT);
		info->data.qc_temp_t2_to_t3_icurrent = QC_CHARGER_CURRENT;
	}

	if (of_property_read_u32(np, "qc_temp_t1_to_t2_icurrent", &val) >= 0)
		info->data.qc_temp_t1_to_t2_icurrent = val;
	else {
		chr_err("use default qc_temp_t1_to_t2_icurrent:%d\n",
			QC_CHARGER_CURRENT);
		info->data.qc_temp_t1_to_t2_icurrent = QC_CHARGER_CURRENT;
	}

	if (of_property_read_u32(np, "qc_temp_t0_to_t1_icurrent", &val) >= 0)
		info->data.qc_temp_t0_to_t1_icurrent = val;
	else {
		chr_err("use default qc_temp_t0_to_t1_icurrent:%d\n",
			QC_CHARGER_CURRENT);
		info->data.qc_temp_t0_to_t1_icurrent = QC_CHARGER_CURRENT;
	}

	if (of_property_read_u32(np, "qc_temp_below_t0_icurrent", &val) >= 0)
		info->data.qc_temp_below_t0_icurrent = val;
	else {
		chr_err("use default qc_temp_below_t0_icurrent:0\n");
		info->data.qc_temp_below_t0_icurrent = 0;
	}

	if (of_property_read_u32(np, "qc_input_current_limit", &val) >= 0)
		info->data.qc_input_current_limit = val;
	else {
		chr_err("use default qc_input_current_limit:%d\n",
			QC_CHARGER_INPUT_CURRENT);
		info->data.qc_input_current_limit = QC_CHARGER_INPUT_CURRENT;
	}

	if (of_property_read_u32(np, "qc_charging_current_limit", &val) >= 0)
		info->data.qc_charging_current_limit = val;
	else {
		chr_err("use default qc_charging_current_limit:%d\n",
			QC_CHARGER_CURRENT);
		info->data.qc_charging_current_limit = QC_CHARGER_CURRENT;
	}
#endif
/*TN End modified by zelin.pan/860620 20230916 CR/EKFOGO4G-2062*/
...
/*TN Begin modified by zhen.liu11/860655 20231018 CR/EKFOGO4G-1548*/
	...
	if (of_find_property(np, "ffc-batt-zone", &byte_len)) {
		if ((byte_len / sizeof(u32)) % 3) {
			pr_err("DT error wrong ffc bat zones\n");
		}

		info->ffc_zones = (struct ffc_bat_zone *)
			devm_kzalloc(dev, byte_len, GFP_KERNEL);

		info->num_ffc_zones =
			byte_len / sizeof(struct ffc_bat_zone);

		if (info->ffc_zones == NULL)
			pr_err("num_ffc_zones is NULL!\n");
...
/*TN Begin modified by zelin.pan/860620 20230823 CR/EKFOGO4G-1548*/
	...
	if ((info->battery_temp >= 35) && (info->battery_temp <= 45)) {
		charger_dev_set_eoc_current(info->chg1_dev, 360000);
	} else {
...
#if IS_ENABLED(QC_CHARGING_MODE)
	if (sw_jeita->sm == TEMP_ABOVE_T4)
		pdata->qc_temp_charging_current_limit = info->data.qc_temp_above_t4_icurrent;
	else if (sw_jeita->sm == TEMP_T3_TO_T4)
		pdata->qc_temp_charging_current_limit = info->data.qc_temp_t3_to_t4_icurrent;
	else if (sw_jeita->sm == TEMP_T2_TO_T3)
		pdata->qc_temp_charging_current_limit = info->data.qc_temp_t2_to_t3_icurrent;
	else if (sw_jeita->sm == TEMP_T1_TO_T2)
		pdata->qc_temp_charging_current_limit = info->data.qc_temp_t1_to_t2_icurrent;
	else if (sw_jeita->sm == TEMP_T0_TO_T1)
		pdata->qc_temp_charging_current_limit = info->data.qc_temp_t0_to_t1_icurrent;
	else if (sw_jeita->sm == TEMP_BELOW_T0)
		pdata->qc_temp_charging_current_limit = info->data.qc_temp_below_t0_icurrent;
	else
		pdata->qc_temp_charging_current_limit = 0;
	chr_err("[SW_JEITA] temp_curr:%d, qc_temp_curr:%d\n",
				pdata->temp_charging_current_limit, pdata->qc_temp_charging_current_limit);
#endif

/*TN Begin modified by hao.jia/809321 20230928 CR/EKFOGO4G-1548*/
	chr_err("[SW_JEITA] preState:%d newState:%d tmp:%d cv:%d\n",
				sw_jeita->pre_sm, sw_jeita->sm, info->battery_temp, sw_jeita->cv);
...
#if IS_ENABLED(CONFIG_FACTORY_BUILD)
static ssize_t factory_charging_limit_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	int value = 0;
	value = factory_charging_limit;
	pr_info("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
	return sprintf(buf, "%d\n", value);
}

static ssize_t factory_charging_limit_write(struct device *dev , struct device_attribute *attr , const char *buf, size_t len)
{

	pr_info("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);

	if (buf[0] == '0') {
		pr_info("%s %d write factory_charging_limit low\n", __FUNCTION__, __LINE__);
		factory_charging_limit = 0;
	} else if (buf[0] == '1') {
		pr_info("%s %d write factory_charging_limit high\n", __FUNCTION__, __LINE__);
		factory_charging_limit = 1;
	} else {
		pr_info("%s %d buf is other number ! ! !\n", __FUNCTION__, __LINE__);
	}
	return 1;
}
static DEVICE_ATTR(factory_charging_limit, 0644, factory_charging_limit_read, factory_charging_limit_write);
#endif
/*TN Begin modified by zelin.pan/860620 20230823 CR/EKFOGO4G-1412*/
static ssize_t battery_protection_mode_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	int value = 0;
	value = battery_protection_mode;
	return sprintf(buf, "%d\n", value);
}

static ssize_t battery_protection_mode_write(struct device *dev , struct device_attribute *attr , const char *buf, size_t len)
{

	pr_info(" %s  enter battery_protection_mode\n", __FUNCTION__);

	if (buf[0] == '0') {
		pr_info("%s write battery_protection_mode low\n", __FUNCTION__);
		battery_protection_mode = 0;
	} else if (buf[0] == '1') {
		pr_info("%s write battery_protection_mode high\n", __FUNCTION__);
		battery_protection_mode = 1;
	} else {
		pr_info("%s buf is other number ! ! !\n", __FUNCTION__);
	}
	return 1;
}
static DEVICE_ATTR(battery_protection_mode, 0644, battery_protection_mode_read, battery_protection_mode_write);
/*TN End modified by zelin.pan/860620 20230823 CR/EKFOGO4G-1412*/

/*TN Begin modified by zelin.pan/860620 20230909 CR/EKFOGO4G-1854*/
static ssize_t demo_mode_limit_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	int value = 0;
	value = demo_mode_limit;
	pr_info("%s line\n", __FUNCTION__);
	return sprintf(buf, "%d\n", value);
}

static ssize_t demo_mode_limit_write(struct device *dev , struct device_attribute *attr , const char *buf, size_t len)
{

	pr_info(" %s  enter demo_mode_limit\n", __FUNCTION__);

	if (buf[0] == '0') {
		pr_info("%s write demo_mode_limit low\n", __FUNCTION__);
		demo_mode_limit = 0;
	} else if (buf[0] == '1') {
		pr_info("%s write demo_mode_limit high\n", __FUNCTION__);
		demo_mode_limit = 1;
	} else {
		pr_info("%s buf is other number ! ! !\n", __FUNCTION__);
	}
...
static DEVICE_ATTR(demo_mode_limit, 0644, demo_mode_limit_read, demo_mode_limit_write);
/*TN End modified by zelin.pan/860620 20230909 CR/EKFOGO4G-1854*/

/*TN Start modified by zelin.pan/860620 20231010 CR/EKFOGO4G-3036*/
static ssize_t turbo_power_mode_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;
	int value = 0;
#if IS_ENABLED(CONFIG_OEM_TURBO_CHARGER)
/*TN Begin modified by zelin.pan/860620 20231201 CR/EKFOGO4G-8482*/
	if (qc3p_charger_ready == 1) {
		turbo_power_mode = 1;
	} else {
		turbo_power_mode = 0;
	}
/*TN End modified by zelin.pan/860620 20231201 CR/EKFOGO4G-8482*/
...
#if IS_ENABLED(QC_CHARGING_MODE)
	if (qc_charging_mode == 1 || turbo_power_mode == 1) {
		turbo_power_mode = 1;
	} else {
		turbo_power_mode = 0;
	}
	...
	if (pinfo->pe5_charging_mode == 1) {
		turbo_power_mode = 1;
	}

	value = turbo_power_mode || turbo_test_mode;
	pr_info("%s value %d, qc3p_charger_ready %d, qc_charging_mode %d, pe5_charging_mode %d\n", __FUNCTION__, value, qc3p_charger_ready, qc_charging_mode, pinfo->pe5_charging_mode);
	return sprintf(buf, "%d\n", value);
}

static ssize_t turbo_power_mode_write(struct device *dev , struct device_attribute *attr , const char *buf, size_t len)
{

	pr_info(" %s  enter turbo_test_mode\n", __FUNCTION__);

	if (buf[0] == '0') {
		pr_info("%s write turbo_test_mode low\n", __FUNCTION__);
		turbo_test_mode = 0;
	} else if (buf[0] == '1') {
		pr_info("%s write turbo_test_mode high\n", __FUNCTION__);
		turbo_test_mode = 1;
	} else {
		pr_info("%s buf is other number ! ! !\n", __FUNCTION__);
	}
...
static DEVICE_ATTR(turbo_power_mode, 0644, turbo_power_mode_read, turbo_power_mode_write);
/*TN End modified by zelin.pan/860620 20231010 CR/EKFOGO4G-3036*/
...
#if IS_ENABLED(CONFIG_FACTORY_BUILD)
	if (factory_charging_limit && (get_uisoc(info) >= 70)) {
		chr_err("TINNO_FACTORY_SUPPORT,soc >= 70 stop charging!!\n");
		charging = false;
		goto stop_charging;
	}
#endif
/*TN Begin modified by zelin.pan/860620 20230823 CR/EKFOGO4G-1412*/
	if (battery_protection_mode) {
		if (get_uisoc(info) >= STOP_CHARGING_SOC) {
			chr_info("enter battery_protection_mode,soc >= 80 stop charging!!\n");
			charging = false;
			goto stop_charging;
		} else if (get_uisoc(info) <= RESUME_CHARGING_SOC) {
			chr_info("exit battery_protection_mode,soc <= 20 start charging!!\n");
			battery_protection_mode = 0;
		} else
			chr_info("in battery_protection_mode,20< soc <= 80 start charging!!\n");
	}
/*TN end modified by zelin.pan/860620 20230823 CR/EKFOGO4G-1412*/

/*TN Begin modified by zelin.pan/860620 20230909 CR/EKFOGO4G-1854*/
	if (demo_mode_limit) {
		if (get_uisoc(info) >= 70) {
			chr_info("enter demo_mode_limit,soc >= 70 stop charging!!\n");
			charging = false;
			goto stop_charging;
		}
	}
...
/*TN Begin modified by zelin.pan/860620 20230916 CR/EKFOGO4G-1548*/
#if IS_ENABLED(QC_CHARGING_MODE)
static int qc_charging(struct mtk_charger *info)
{
	...
	ret = get_vbus(info);
	if (ret < 0) {
		chr_err("%s get vbus voltage failed before qc3.0 check\n", __func__);
		return ret;
	} else {
		vbus_volt_old = ret;
	}

	if (vbus_volt_old >= QC_TARGE_VOLT) {
		chr_err("%s vbus_volt_old qc3.0 detected, vbus old voltage: %d\n", __func__, vbus_volt_old);
		return ret;
	}
...
/*TN Begin modified by maocai.cao/808964 20231124 CR/EKFOGO4G-3815*/
	if (ret < 0) {
		chr_err("%s ignore qc3.0 detect due to set dp voltage failed(%d)\n", __func__, ret);
		return ret;
	}
...
/*TN Begin modified by maocai.cao/808964 20231124 CR/EKFOGO4G-3815*/
	if (ret < 0) {
		chr_err("%s ignore qc3.0 detect due to set dm voltage failed(%d)\n", __func__, ret);
		return ret;
	}
/*TN End modified by maocai.cao/808964 20231124 CR/EKFOGO4G-3815*/

	chr_info("%s enter qc3.0 vbus old %d\n", __func__, vbus_volt_old);
	msleep(100);
	for (i = 0; i < 15; i++) {
		ret = charger_dev_set_dp_voltage(info->chg1_dev, 3300000);
/*TN Begin modified by maocai.cao/808964 20231124 CR/EKFOGO4G-3815*/
		if (ret < 0) {
			chr_err("%s ignore qc3.0 detect due to set dp voltage failed(%d), Loop: %d\n", __func__, ret, i);
			return ret;
		}
/*TN End modified by maocai.cao/808964 20231124 CR/EKFOGO4G-3815*/
...
/*TN End modified by maocai.cao/808964 20231124 CR/EKFOGO4G-3815*/
		...
		while (time_out < 100) {
			...
			ret = get_vbus(info);
			if (ret < 0) {
				chr_err("%s check qc3.0 vbus failed, try count: %d\n", __func__, i);
				return ret;
			}
			vbus_volt_new = get_vbus(info);
			if (vbus_volt_new > vbus_volt_old) {
				vbus_volt_old = vbus_volt_new;
				chr_info("%s qc3.0 detected, vbus new voltage: %d\n",  __func__, vbus_volt_new);
				break;
			}
			time_out++;
		}
		if (time_out == 100) {
			chr_info("%s qc3.0 not supported\n", __func__);
			return -EINVAL;
		}

		if (vbus_volt_new >= QC_TARGE_VOLT) {
			chr_err("%s vbus new >= %d, vbus voltage: %d\n", __func__, QC_TARGE_VOLT, vbus_volt_new);
			qc_charging_mode = 1;
			break;
		}
	}

	if (vbus_volt_new >= QC_MAX_VOLT) {
		msleep(300);
		...
		chr_err("%s vbus new > %d , vbus voltage: %d\n", __func__, QC_MAX_VOLT, vbus_volt_new);
	}
...
static int qc3_charger_detect_notifier_cb(struct notifier_block *nb,
			unsigned long event, void *data)
{
	struct mtk_charger *info = container_of(nb,
						struct mtk_charger, qc3_charger_detect_nb);
	...
	if (psy == info->chg_psy || psy == info->qc_logic_psy) {
		...
		if (first_insert) {
			...
			if (ret < 0) {
				...
				if (chr_type == POWER_SUPPLY_USB_TYPE_DCP) {
					if (!oem_pcba_chg_15w_exist()) {
						chr_err("%s: found 33W device, is_qc3_charger_ready:%d\n", __func__, is_qc3_charger_ready);
						if (is_qc3_charger_ready) {
							chr_err("%s: detect qc3.0 charger, try to tuning voltage\n", __func__);
							charger_dev_set_dp_voltage(info->chg1_dev, 600000);
							schedule_delayed_work(&info->hvdcp_work, msecs_to_jiffies(1500));
							first_insert = false;
						}
					} else {
						chr_err("%s: found 15W device, try to detect qc3.0 charger\n", __func__);
						charger_dev_set_dp_voltage(info->chg1_dev, 600000);
						schedule_delayed_work(&info->hvdcp_work, msecs_to_jiffies(1500));
						first_insert = false;
					}
		...
		struct work_struct *work)
{
	...
	int ret;
	chr_err("%s enter qc3.0 detect work\n", __func__);
	...
	int batt_ma, int taper_ma)
{
	...
	pr_info("[%s] curr target_ma = %d batt_ma = %d\n", __func__, target_ma, batt_ma);
...
/*TN Begin modified by zelin.pan/860620 20231114 CR/EKFOGO4G-3272*/
		//check_dynamic_mivr(info);
/*TN End modified by zelin.pan/860620 20231114 CR/EKFOGO4G-3272*/
...
/*TN Begin modified by zhen.liu11/860655 20231121 CR/EKFOGO4G-1548*/
#if IS_ENABLED(CONFIG_OEM_TURBO_CHARGER)
				if (turbo_charger_active == true
					&& info->sw_jeita.sm == TEMP_T2_TO_T3
					/*TN Begin modified by lingfei.tang11/777407 20231129 CR/EKFOGO4G-8389*/
					...
					/*TN End modified by lingfei.tang11/777407 20231129 CR/EKFOGO4G-8389*/
...
/*TN End modified by zhen.liu11/860655 20231121 CR/EKFOGO4G-1548*/
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	if (ret)
		goto _out;
 #if IS_ENABLED(CONFIG_FACTORY_BUILD)
	ret = device_create_file(&(pdev->dev), &dev_attr_factory_charging_limit);
	if (ret)
		goto _out;
#endif
/*TN Begin modified by zelin.pan/860620 20230823 CR/EKFOGO4G-1412*/
...
/*TN Begin modified by zelin.pan/860620 20231010 CR/EKFOGO4G-3036*/
...
/*TN Begin modified by hao.jia/809321 20231103 CR/EKFOGO4G-1548*/
		...
		chr_info("%s: on no thermal build, ignore thermal limit charge current\n", __func__);
#else
		/*TN add begin by chao.zhang1/860682 20230926 CR/EKFOGO4G-1785*/
...
/*TN Begin modified by hao.jia/809321 20231103 CR/EKFOGO4G-1548*/
#ifdef CONFIG_NO_THERMAL_BUILD
		chr_info("%s: on no thermal build, ignore thermal limit input current\n", __func__);
#else
...
/*TN Begin modified by hao.jia/809321 2023814 CR/EKFOGO4G-1483*/
		...
		chg_psy = power_supply_get_by_name("ext_charger_type");
		if (IS_ERR_OR_NULL(chg_psy)) {
			pr_err("%s get ext_charger_type chg psy failed\n", __func__);
		}
#else
		chg_psy = devm_power_supply_get_by_phandle(&info->pdev->dev,
						       "charger");
...
/*TN Begin modified by hao.jia/809321 2023814 CR/EKFOGO4G-1483*/
#if IS_ENABLED(CONFIG_OEM_SWITCH_CHARGER_SC89890H) || IS_ENABLED(CONFIG_OEM_SWITCH_CHARGER_SGM41542)
	info->chg_psy = power_supply_get_by_name("ext_charger_type");
...
/*TN Begin modified by zelin.pan/860620 20230916 CR/EKFOGO4G-2062*/
	...
	info->qc3_charger_detect_nb.notifier_call = qc3_charger_detect_notifier_cb;
	power_supply_reg_notifier(&info->qc3_charger_detect_nb);
#endif
/*TN End modified by zelin.pan/860620 20230916 CR/EKFOGO4G-2062*/
