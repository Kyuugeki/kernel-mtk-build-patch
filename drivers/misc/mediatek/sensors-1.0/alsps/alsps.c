...
static ssize_t proxcali_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	struct alsps_context *cxt = NULL;
	int cali_flag = 0, err = 0;

	err = kstrtoint(buf, 10, &cali_flag);
	if (err != 0)
		pr_err("%s param error: err = %d\n", __func__, err);

	pr_err("%s param: cali_flag= %d\n", __func__, cali_flag);

	//mutex_lock(&alsps_context_obj->alsps_op_mutex);
	if (cali_flag == 1) {
	cxt = alsps_context_obj;
	if (cxt->ps_ctl.send_cali != NULL) {
		pr_err("%s param: send_cali enter\n", __func__);
		err = cxt->ps_ctl.send_cali();
		if (err < 0)
		pr_err("%s ps enable cali err %d\n", __func__, err);
	}
...
int ps_register_control_path(struct ps_control_path *ctl)
{
	...
	//TN modified by ye.yuan/860593 20240102 EKFOGO4G-9716 begin
	cxt->ps_ctl.send_cali = ctl->send_cali;
	//TN modified by ye.yuan/860593 20240102 EKFOGO4G-9716 end
