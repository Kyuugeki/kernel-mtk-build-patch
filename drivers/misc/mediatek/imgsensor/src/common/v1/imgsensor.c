...
static void get_subcam_hwid(struct device_node *np)
{
    int front_id_gpio = 0;
    front_id_gpio = of_get_named_gpio(np, "tinno,frontid-gpio", 0);
    if (!gpio_is_valid(front_id_gpio)) {
        pr_err("get gpio failed!\n");
    }

    if (gpio_request(front_id_gpio, "front_hwid") < 0) {
        pr_err("gpio request failed\n");
    }

    if (gpio_direction_input(front_id_gpio) < 0) {
        pr_err("set gpio direction failed!\n");
    }

    frontHwId = gpio_get_value(front_id_gpio);
    pr_info("Fogo %s gpio158 = %d hwid = %d \n", __func__, front_id_gpio, frontHwId);
    gpio_free(front_id_gpio);
}
...
static int imgsensor_probe(struct platform_device *pdev)
{
    ...
    get_subcam_hwid(pdev->dev.of_node);
