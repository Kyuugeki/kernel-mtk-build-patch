	...
	char *pcurr_idx)
{
	...
	while (ppwr_info->pin != IMGSENSOR_HW_PIN_NONE &&
		 ...
		 */
            ...
            if (ppwr_info->pin == IMGSENSOR_HW_PIN_AVDD && pdev->set != NULL && sensor_idx == 2) {
				pr_info("marco avdd on");
	...
	enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	...
	pr_info(
		"Fogo HwID = %d sensor_idx %d, power %d curr_sensor_name %s, enable list %s\n",
		frontHwId,
		sensor_idx,
		pwr_status,
		curr_sensor_name,
	...
	} else if (1 == frontHwId) { // 16M
	    imgsensor_hw_power_sequence(
			    phw,
			    sensor_idx,
			    pwr_status, sensor_power_secondary_sequence, curr_sensor_name);
	} else { // default 8M
	    imgsensor_hw_power_sequence(
			    phw,
			    sensor_idx,
