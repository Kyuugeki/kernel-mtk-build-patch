...
static void preview_setting(void)
{
	kal_uint8 framecnt;
	int i;
	CAM_DBG(PFX,"preview_setting() E\n");
	write_cmos_sensor_8(0x0100, 0x00);
	for (i = 0; i < 100; i++) {
		framecnt = read_cmos_sensor(0x0005);
		if (framecnt == 0xFF) {
			CAM_DBG(PFX,"stream is off\n");
			break;
		} else {
			CAM_DBG(PFX,"stream is not off\n");
			mdelay(1);
		}
	}
	...
	write_cmos_sensor_8(0x030F, 0xAE);
	...
	write_cmos_sensor_8(0x0821, 0xB8);
...
static void capture_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor_8(0x0203, 0x02);
	write_cmos_sensor_8(0x3400, 0x01);
	CAM_DBG(PFX,"capture_setting End\n");
}
...
static void normal_video_setting(kal_uint16 currefps)
{
	kal_uint8 framecnt;
	int i;
	CAM_DBG(PFX,"normal_video_setting() E! currefps:%d\n", currefps);
	write_cmos_sensor_8(0x0100, 0x00);
...
static void hs_video_setting(void)
{
	kal_uint8 framecnt;
	int i;
	CAM_DBG(PFX,"hs_video_setting() E\n");
...
static void slim_video_setting(void)
{
	kal_uint8 framecnt;
	int i;
	CAM_DBG(PFX,"slim_video_setting() E\n");
...
#define MODULE_ID_START_ADD 0x0a12
#define MODULE_ID_LENGTH 0x02
#define getbit(x,y)   ((x) >> (y)&1)
#define DUMP_DATA_PATH "/data/vendor/camera_dump/s5k4h7sub_otp_data.bin"

struct s5k4h7_otp_t s5k4h7_otp_info = {0};
EXPORT_SYMBOL(s5k4h7_otp_info);
...
static int read_otp_page_data(int page, int start_add, unsigned char *Buff, int size)
 {
	...
	for ( i = 0; i < size; i++ ) {
		...
		CAM_DBG(PFX, "cur page = %d, Buff[%d] = 0x%x\n",page,i,Buff[i]);
	}
...
static bool read_valid_data(int page, int start_add)
 {
	...
	if (page_flag[0] != 0 || page_flag[1] != 0) {
		s5k4h7_otp_info.page_flag = ((page_flag[0] > page_flag[1])? page_flag[0] : page_flag[1]);
		if (!((getbit(s5k4h7_otp_info.page_flag,6)) && !(getbit(s5k4h7_otp_info.page_flag,7)))) {
			LOG_INF("valid bit 7 = %d,bit 6 = %d\n",getbit(s5k4h7_otp_info.page_flag,7),getbit(s5k4h7_otp_info.page_flag,6));
...
static int read_otp_data(void)
 {
	...
	read_otp_page_data(page, 0x0a1a, s5k4h7_otp_info.serial_number, 8);
	/* read_otp_page_data(page, 0x0a12, s5k4h7_otp_info.name, 2); */
	//read module id

	read_otp_page_data(page, MODULE_ID_START_ADD, s5k4h7_otp_info.module_id, MODULE_ID_LENGTH);
	for (i = 0;i < MODULE_ID_LENGTH ; i++ ) {
		LOG_INF ("+++4h7 6 page = %d modulue_id[%d] = 0x%x",page,i,s5k4h7_otp_info.module_id[i]);
		sum += s5k4h7_otp_info.module_id[i];
	}
	if (!sum) return -1;
	...
	read_otp_page_data(page, 0x0a3d, s5k4h7_otp_info.awb_gloden, 7);
	read_otp_page_data(page + 1, 0x0a04, s5k4h7_otp_info.awb_gloden + 7, 1);
	read_otp_page_data(page + 1, 0x0a19, s5k4h7_otp_info.awb_unint, 8);
	...
	if(page == 36) {
		for (i = 0; i < 30 ; i++ ) {
			if (i == 29) {
				read_otp_page_data(page + i, lsc_addr, s5k4h7_otp_info.lscdata + (64*i), 15);
			} else {
				read_otp_page_data(page + i, lsc_addr, s5k4h7_otp_info.lscdata + (64*i), 64);
			}
		}
	} else if(page == 65) {
		lsc_addr = 0x0a13;
		for (i = 0; i < 30 ; i++ ) {
			if( (i!=0) && (i!=29)) {
				read_otp_page_data(page + i, 0x0a04, s5k4h7_otp_info.lscdata + (49 + 64*(i-1)), 64);
			} else if (i == 0) {
				read_otp_page_data(page + i, lsc_addr, s5k4h7_otp_info.lscdata + (64*i), 49);
			} else if(i == 29){
				read_otp_page_data(page + i, 0x0a04, s5k4h7_otp_info.lscdata + (49 + 64*(i-1)), 30);
			}
		}
	} else if(page == 94) {
		lsc_addr = 0x0a22;
		for (i = 0; i < 30 ; i++ ) {
			if( (i!=0) && (i!=29)) {
				read_otp_page_data(page + i, 0x0a04, s5k4h7_otp_info.lscdata + (34 + 64*(i-1)), 64);
			} else if (i == 0) {
				read_otp_page_data(page + i, lsc_addr, s5k4h7_otp_info.lscdata + (64*i), 34);
			} else if(i == 29){
				read_otp_page_data(page + i, 0x0a04, s5k4h7_otp_info.lscdata + (34 + 64*(i-1)), 45);
			}
		}
	}
	checksum = GetCRC(s5k4h7_otp_info.lscdata, 1869);
	datasum = s5k4h7_otp_info.lscdata[1869] * 256 + s5k4h7_otp_info.lscdata[1870];
	if ((checksum == datasum) && checksum !=0 && datasum !=0) {
		s5k4h7_otp_info.lscCheckSum = 1;
		LOG_INF("lsc data check succ");
	} else {
		s5k4h7_otp_info.lscCheckSum = 0;
		LOG_INF("lsc data check failed");
	}
	LOG_INF("lsc checksum = %d, data-chk = %d", checksum, datasum);
	return 0;
 }

#endif //READ_SENSOR_OTP
...
int front_8m_cam_get_info(char *buf, void *arg0)
{
	long resolv = 0;
	int pi = 0;
	...
	if (s5k4h7_otp_info.module_id[0] == 0x53 && s5k4h7_otp_info.module_id[1] == 0x57) {
	    return sprintf(buf, "%s [%d*%d] %dM", "s5k4h7_front_sw_|_mipi_raw", imgsensor_info.cap.grabwindow_width, imgsensor_info.cap.grabwindow_height, pi);
	} else if (s5k4h7_otp_info.module_id[0] == 0x53 && s5k4h7_otp_info.module_id[1] == 0x55){
	    return sprintf(buf, "%s [%d*%d] %dM", "s5k4h7_front_sn_||_mipi_raw", imgsensor_info.cap.grabwindow_width, imgsensor_info.cap.grabwindow_height, pi);
	} else {
		return sprintf(buf, "%s [%d*%d] %dM", "s5k4h7_front_mipi_raw", imgsensor_info.cap.grabwindow_width, imgsensor_info.cap.grabwindow_height, pi);
	}
}
#endif
...
#if IS_ENABLED(CONFIG_OEM_DEVINFO)
				FULL_PRODUCT_DEVICE_CB(ID_FRONT1_CAM, front_8m_cam_get_info, NULL);
#endif
                   break;
			}
			retry--;
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	CAM_DBG(PFX,"feature_id = %d", feature_id);
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
            CAM_DBG(PFX,"feature_Control imgsensor.pclk = %d,imgsensor.current_fps = %d\n", imgsensor.pclk, imgsensor.current_fps);
		...
		case SENSOR_FEATURE_SET_HDR:
			CAM_DBG(PFX,"Warning! Not Support IHDR Feature");
			spin_lock(&imgsensor_drv_lock);
