...
#include "sc202csmipiraw_Sensor.h"
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
    ...
    CAM_DBG(PFX,"framerate = %d, min framelength should enable? \n", framerate,min_framelength_en);
...
static void write_shutter(kal_uint16 shutter)
{
	...
	//write_cmos_sensor(0x3812,0x30);//group hold off   //group hold
        CAM_DBG(PFX,"gsl_debug shutter =%d, framelength =%d,ExpReg 0x3e00=%x, 0x3e01=%x, 0x3e02=%x\n", shutter,imgsensor.frame_length,(shutter >> 12) & 0x0F,(shutter >> 4)&0xFF, (shutter<<4) & 0xF0);
	//CAM_DBG(PFX,"ExpReg 0x3e00=%x, 0x3e01=%x, 0x3e02=%x\n",(shutter >> 12) & 0x0F,(shutter >> 4)&0xFF, (shutter<<4) & 0xF0);
}   /*  write_shutter  */
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length)
{
	...
	imgsensor.shutter = shutter;
	spin_unlock_irqrestore(&imgsensor_drv_lock, flags);
	/* CAM_DBG(PFX,"shutter =%d, frame_time =%d\n", shutter, frame_time); */
	...
	write_cmos_sensor(0x3e01, (shutter >> 4)&0xFF);
	write_cmos_sensor(0x3e02, (shutter<<4) & 0xF0);	
	CAM_DBG(PFX,"Exit! shutter =%d, framelength =%d/%d, dummy_line=%d, \n", shutter,
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    CAM_DBG(PFX,"Gain_Debug pass_gain= %d\n",gain);
...
{
	CAM_DBG(PFX,"capture_setting E! currefps:%d\n",currefps);
    write_cmos_sensor(0x0103,0x01);
    write_cmos_sensor(0x0100,0x00);
    write_cmos_sensor(0x36e9,0x80);
    write_cmos_sensor(0x36e9,0x24);
...
{
	CAM_DBG(PFX,"normal_video_setting currefps:%d\n",currefps);
    write_cmos_sensor(0x0103,0x01);
    write_cmos_sensor(0x0100,0x00);
    write_cmos_sensor(0x36e9,0x80);
    write_cmos_sensor(0x36e9,0x24);
...
{
	CAM_DBG(PFX,"E! VGA 120fps\n");
}

static void slim_video_setting()
{
	CAM_DBG(PFX,"E! HD 30fps\n");
...
#ifdef ENABLE_OTP
struct sc202cs_otp_t sc202cs_otp_info = {0};
EXPORT_SYMBOL_GPL(sc202cs_otp_info);

static bool sc202cs_sensor_otp_init() {
    int delay=0;

    write_cmos_sensor(0x3106,0x05);
    write_cmos_sensor(0x440d,0x10);
    write_cmos_sensor(0x4409,0x4400);
    write_cmos_sensor(0x440b,0x4411);
    write_cmos_sensor(0x0100,0x01);
    write_cmos_sensor(0x4400,0x11);

    //msleep(5);
    while((read_cmos_sensor(0x4411)&0x01) == 0x01){
        delay++;
        if(delay == 1000) {
...
static bool SC202CS_otp_read_group(kal_uint16 addr, kal_uint8 *data, kal_uint8 length)
{
    kal_uint8 i = 0;
    for (i = 0; i < length; i++) {
...
bool sc202cs_iReadData(unsigned int ui4_offset,
    unsigned int ui4_length, unsigned char *pinputdata)
{
    bool i4RetValue = true;

    CAM_DBG(PFX,"sc202cslycniotp ui4_offset = 0x%x, ui4_length = %d \n", ui4_offset, ui4_length);

    CAM_DBG(PFX,"offset = 0x%x, length = %d \n", ui4_offset, ui4_length);
    mdelay(10);
...
static uint16_t GetCRC(const uint8_t *data, size_t len) {
	int crc[16] = { 0 };
    ...
    for (i = 0; i < 16; i++)
    {
        mycrc16x = mycrc16x + (crc[i] << i);
    }
    return mycrc16x;
}

bool sc202cs_check_otp_info(void)
{
    uint8_t calchecksumdata[12];
    uint8_t calchecksumdata_group2[12];
    uint16_t calchecksum = 0;
    uint16_t burnchecksum = 0;
    bool ret = true;
#define PRINT_OTP_DATA 1
#if PRINT_OTP_DATA
    int i = 0;
#endif
    ret = sc202cs_sensor_otp_init();
    if (ret != true) {
        CAM_DBG(PFX,"sensor otp init failed");
        return ret;
    }

    // read data flag
    ret = sc202cs_iReadData(0x8008, 1, &sc202cs_otp_info.dataFlag);

    // read module info
    if (((sc202cs_otp_info.dataFlag >> 6) & 0x3) == 0x01) {
        CAM_DBG(PFX,"group 1 valid");
        ret = sc202cs_iReadData(0x8009, 3, sc202cs_otp_info.awb_golden_param);
        ret = sc202cs_iReadData(0x8011, 3, sc202cs_otp_info.awb_param);
        burnchecksum = read_cmos_sensor(0x8014)*256 + read_cmos_sensor(0x8015);
        sc202cs_iReadData(0x8008, 12, calchecksumdata);
        calchecksum = GetCRC(calchecksumdata, 12);
        CAM_DBG(PFX,"sc202cs calChksum: 0x%x", calchecksum);
        CAM_DBG(PFX,"sc202cs burnChksum: 0x%x", burnchecksum);
    } else if (((sc202cs_otp_info.dataFlag >> 4) & 0x3) == 0x01) {
        CAM_DBG(PFX,"group 2 valid");
        ret = sc202cs_iReadData(0x8016, 3, sc202cs_otp_info.awb_golden_param);
        ret = sc202cs_iReadData(0x8019, 3, sc202cs_otp_info.awb_param);
        burnchecksum = read_cmos_sensor(0x801D)*256 + read_cmos_sensor(0x801F);
        sc202cs_iReadData(0x80016, 7, calchecksumdata_group2);
        calchecksum = GetCRC(calchecksumdata_group2, 7);
        CAM_DBG(PFX,"sc202cs calChksum: 0x%x", calchecksum);
        CAM_DBG(PFX,"sc202cs burnChksum: 0x%x", burnchecksum);
    } else if ((((sc202cs_otp_info.dataFlag >> 6) & 0x3) == 0x00) && (((sc202cs_otp_info.dataFlag >> 4) & 0x3) == 0x00)) {
        CAM_DBG(PFX,"no otp data");
        ret = false;
        return ret;
    } else {
        CAM_DBG(PFX,"otp data invalid");
        ret = false;
        return ret;
    }

    if (calchecksum == burnchecksum && calchecksum != 0 && burnchecksum != 0)
    {
        sc202cs_otp_info.dataChksum = 1;
        CAM_DBG(PFX,"sc202cs awb checksum pass!");
    }else {
        CAM_DBG(PFX,"sc202cs awb checksum failed!");
        ret = false;
        return ret;
    }
...
#if PRINT_OTP_DATA
    for (i = 0; i < 3; i++) {
        CAM_DBG(PFX,"sc202cs awb golden info: 0x%x", sc202cs_otp_info.awb_golden_param[i]);
    }
    for (i = 0; i < 3; i++) {
        CAM_DBG(PFX,"sc202cs awb info: 0x%x", sc202cs_otp_info.awb_param[i]);
    }
    CAM_DBG(PFX,"sc202cs check sum: 0x%x", sc202cs_otp_info.dataChksum);
#endif
...
int macro_cam_get_info(char *buf, void *arg0)
{
	long resolv = 0;
	int pi = 0;
    ...
    if ((sc202cs_otp_info.dataFlag & 1) == 1)
    {
        return sprintf(buf, "%s [%d*%d] %dM", "sc202cs_macro_sn_|||_mipi_raw", imgsensor_info.cap.grabwindow_width, imgsensor_info.cap.grabwindow_height, pi);
    } else {
        return sprintf(buf, "%s [%d*%d] %dM", "sc202cs_macro_mipi_raw", imgsensor_info.cap.grabwindow_width, imgsensor_info.cap.grabwindow_height, pi);
    }
}
#endif
...
#ifdef ENABLE_OTP
                if(sc202cs_check_otp_info()) {
                    LOG_INF("sc202cs_check_otp_info pass");
                }
#endif

#if IS_ENABLED(CONFIG_OEM_DEVINFO)
				FULL_PRODUCT_DEVICE_CB(ID_MAIN2_CAM, macro_cam_get_info, NULL);
#endif
        ...
        case SENSOR_FEATURE_GET_CROP_INFO:
            ...
            switch (*feature_data_32) {
                ...
                case MSDK_SCENARIO_ID_SLIM_VIDEO:
                    ...
                    break;
		case MSDK_SCENARIO_ID_CUSTOM1:
			CAM_DBG(PFX,"lch MSDK_SCENARIO_ID_CUSTOM1 \n");
			memcpy(
				(void *)wininfo,
