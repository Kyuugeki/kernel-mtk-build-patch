...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
 	{4640,3488,		  0,	 0, 4640, 3488, 2320, 1744,
	 	0,	 0,		2320,	1744,	0,	 2, 2320, 1740},/*Preview*/
 	{4640,3488,		  0,	 0, 4640, 3488, 4640, 3488,
	 	0,	 0,		4640,	3488,	0,	 0, 4640, 3488},/*capture remosic*/
 	{4640,3488,		  0,	 0, 4640, 3488, 2320, 1744,
	 	0,	 0,		2320,	1744,	0,	 2, 2320, 1740},/*video*/
	...
	{4640, 3488,      0,     0, 4640, 3488, 2320, 1744,
	    0,    0,   2320,  1744,    0,    2, 2320, 1740},/*custom1*/
...
static void read_eeprom_SN(void)
{
	...
	for (idx = 0; idx < CAMERA_MODULE_SN_LENGTH; idx++) {
		...
		CAM_DBG(PFX,"gS5k3p9sp_SN[%d]: 0x%x  0x%x\n",
...
static bool Remosaicmode = false;
	...
	kal_uint16 shutter, kal_uint16 frame_length)
{
	...
	CAM_DBG(PFX,"shutter = %d, framelength = %d/%d, dummy_line= %d\n",
		shutter, imgsensor.frame_length,
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		...
		for (i = 0; i < timeout; i++) {
			...
			if (framecnt == 0xFF) {
				CAM_DBG(PFX," Stream Off OK at i=%d.\n", i);
				return ERROR_NONE;
			}
		}
		CAM_DBG(PFX,"Stream Off Fail! framecnt= %d.\n", framecnt);
	}
	return ERROR_NONE;
}
...
{
	CAM_DBG(PFX,"capture_setting start\n");
...
UINT32 S5K3P9SP_Get_Module_Id(void)
{
	imgsensor.i2c_write_id = 0xA2;
	moduleid = ((read_cmos_sensor_16_8(0x000D) << 8) | read_cmos_sensor_16_8(0x000E));
...
#define FOUR_CELL_SIZE 2048/*size = 2048 = 0x800*/
static char Four_Cell_Array[FOUR_CELL_SIZE + 2];
static void get_xtc_data(){
	int i = 0;
	...
	CAM_DBG(PFX,"copy_4cell_from_eeprom ++\n");
	for (i = 0; i < 2048; ++i)
	{
		Four_Cell_Array[i + 2] = read_cmos_sensor_16_8(0x0F44 + i);
		CAM_DBG(PFX,"xtcdata:%d,%x \n", i + 2, Four_Cell_Array[i + 2]);
	}
	checksum = GetCRC(Four_Cell_Array + 2, 2048);
	datasum = read_cmos_sensor_16_8(0x1744) << 8 | read_cmos_sensor_16_8(0x1745);
	CAM_DBG(PFX,"calchecksum:%x datachecksum:%x", checksum, datasum);
}
...
static void copy_4cell_from_eeprom(char *data)
{
	if (data != NULL) {
		...
		CAM_DBG(PFX,"xtcdata_length:%d \n", sizeof(Four_Cell_Array) / sizeof(Four_Cell_Array[0]));
	}
	CAM_DBG(PFX,"copy_4cell_from_eeprom --\n");
}

#if IS_ENABLED(CONFIG_OEM_DEVINFO)
int front_16m_cam_get_info(char *buf, void *arg0)
{
	long resolv = 0;
	int pi = 0;
	...
	if (moduleid == 0x5154) {
		return sprintf(buf, "%s [4608*3456] 16M", "s5k3p9sp_front_qt_|_mipi_raw");
	} else if (moduleid == 0x5357) {
		return sprintf(buf, "%s [4608*3456] 16M", "s5k3p9sp_front_sw_||_mipi_raw");
	} else {
		return sprintf(buf, "%s [4608*3456] 16M", "s5k3p9sp_front_mipi_raw");
	}
}
#endif
			...
			*sensor_id =
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				*sensor_id = S5K3P9SP_SENSOR_ID;
				...
				#endif
#if IS_ENABLED(CONFIG_OEM_DEVINFO)
				FULL_PRODUCT_DEVICE_CB(ID_FRONT1_CAM, front_16m_cam_get_info, NULL);
#endif
				return ERROR_NONE;
			}
...
static kal_uint32 open(void)
{
	...
	CAM_DBG(PFX,"PLATFORM:MT6750,MIPI 4LANE\n");
	CAM_DBG(PFX,"preview 1280*960@30fps,864Mbps/lane;");
	CAM_DBG(PFX,"video 1280*960@30fps,864Mbps/lane;");
	CAM_DBG(PFX,"capture 5M@30fps,864Mbps/lane\n");
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps != imgsensor_info.cap.max_framerate) {
		CAM_DBG(PFX,"Warning: current_fps %d fps is not support",
			imgsensor.current_fps);
		CAM_DBG(PFX,"so use cap's setting: %d fps!\n",
	...
	struct SET_SENSOR_PATTERN_SOLID_COLOR *pdata)
{
	...
	LOG_INF("set_test_pattern1 enum: %d\n", modes);

	if (modes) {
		if (modes == 1 && (pdata != NULL)) { //Solid Color
			write_cmos_sensor_16_16(0x0600, 0x0001);
			Color_R = (pdata->COLOR_R) & 0xFFFF;
			Color_Gr = (pdata->COLOR_Gr) & 0xFFFF;
			Color_B = (pdata->COLOR_B) & 0xFFFF;
			Color_Gb = (pdata->COLOR_Gb) & 0xFFFF;
			write_cmos_sensor_16_16(0x0602, Color_Gr);
			write_cmos_sensor_16_16(0x0604, Color_R);
			write_cmos_sensor_16_16(0x0606, Color_B);
			write_cmos_sensor_16_16(0x0608, Color_Gb);
		}
	} else {
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_MODULE_SN:
		CAM_DBG(PFX,"s5k3p9 GET_MODULE_SN:%d %d\n",
	 ...
	 *CAM_DBG(PFX,"SENSOR_FEATURE_GET_PDAF_DATA\n");
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		CAM_DBG(PFX,"SENSOR_FEATURE_GET_CROP_INFO:");
		CAM_DBG(PFX,"scenarioId:%d\n", *feature_data_32);
		wininfo =
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		CAM_DBG(PFX,"SENSOR_FEATURE_GET_PDAF_INFO");
		CAM_DBG(PFX,"scenarioId:%lld\n", *feature_data);
		PDAFinfo =
	...
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
		CAM_DBG(PFX,"SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY");
		CAM_DBG(PFX,"scenarioId:%lld\n", *feature_data);
 ...
 *		CAM_DBG(PFX,"SENSOR_FEATURE_GET_CUSTOM_INFO information :);
 *		CAM_DBG(PFX,"type:%lld  S5K39PSP04_OTP_ERROR_CODE:%d\n",
 ...
 *			#if 0
 *			for (i = 0 ; i<12 ; i++ ){
 *			CAM_DBG(PFX,"sn_inf_sub_S5K3P9SP[%d]= 0x%x\n",
 ...
 */
	...
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		CAM_DBG(PFX,"SENSOR_FEATURE_SET_STREAMING_RESUME");
		CAM_DBG(PFX,"shutter:%llu\n", *feature_data);
		if (*feature_data != 0)
			set_shutter(*feature_data);
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:
		...
		if (type == FOUR_CELL_CAL_TYPE_ALL) {
			CAM_DBG(PFX,"SENSOR_FEATURE_GET_4CELL_DATA type=%d\n"
				, type);
			copy_4cell_from_eeprom((char *)(*(feature_data+1)));
			...
			CAM_DBG(PFX,"No type %d buffer on this sensor\n", type);
		}
		break;
		}
