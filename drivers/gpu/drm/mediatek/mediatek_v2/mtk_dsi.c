...
/* TN Begin modified by kexin.wang/860557 20231102 CR/EKFOGO4G-6385 */
...
/* TN end modified by kexin.wang/860557 20231102 CR/EKFOGO4G-6385*/
...
pr_info("%s: dsi->vfp=%d, dsi->vbp=%d, dsi->vsa=%d , dsi->data_rate=%d\n",
				__func__, dsi->vfp, dsi->vbp, dsi->vsa, dsi->data_rate);
}
...
{
	unsigned int ret = 0;
	/* TN Begin modified by kexin.wang/860557 20231127 CR/EKFOGO4G-8433*/
	struct mtk_drm_crtc *mtk_crtc = dsi->is_slave ?
		dsi->master_dsi->ddp_comp.mtk_crtc
		: dsi->ddp_comp.mtk_crtc;
	struct mtk_drm_private *priv = NULL;
	...
	if (mtk_crtc && mtk_crtc->base.dev)
		...
		priv = dsi->encoder.dev->dev_private;
	/* TN end modified by kexin.wang/860557 20231127 CR/EKFOGO4G-8433*/
...
static int mtk_dsi_probe(struct platform_device *pdev)
{
	...
	int irq_num;
	int comp_id;
	bool panel_lock = false; // TN modified by kexin.wang/860557 20230824 CR/EKFOGO4G-1553
	int ret;
	...
	mtk_panel_lock(); // TN modified by kexin.wang/860557 20230824 CR/EKFOGO4G-1553
	ret = mipi_dsi_host_register(&dsi->host);
	if (ret < 0) {
		dev_err(dev, "failed to register DSI host: %d\n", ret);
		mtk_panel_unlock(); // TN modified by kexin.wang/860557 20230824 CR/EKFOGO4G-1553
		return -EPROBE_DEFER;
	}
	...
	if (!dsi->is_slave) {
		...
		if (endpoint) {
			...
			if (!remote_node) {
				...
				panel_lock = true; // TN modified by kexin.wang/860557 20230824 CR/EKFOGO4G-1553
				ret = -ENODEV;
				goto error;
			}
